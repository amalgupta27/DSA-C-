...
int main() {
    ...
    while (l <= h) { 
        ...
        if (arr[mid] == x) { // If the mid element is equal to x
            if (arr[mid + 1] != x) { // Check if it is the last occurrence of x
                cout << "Element found at index " << mid << endl; // Print the index of last occurrence
                flag = true; // Set flag to true since the element is found
                break; // Exit the loop after finding the last occurrence
            }
            if (arr[mid + 1] == x) l = mid + 1; // Move right to find the last occurrence
        }
